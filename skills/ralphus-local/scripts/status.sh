#!/bin/bash
# status.sh - Returns the current state of the Ralphus environment
...
# Check if plan covers specs
PLAN_COMPLETE=false
if [ "$PLAN_EXISTS" = true ] && [ "$SPECS_EXIST" = true ]; then
    # Simple heuristic: Does plan mention spec files?
...
if [ "$RUNNING" = true ]; then
    ...
    echo "NEXT: Launch BUILD mode (launch.sh build)"
else
    echo "NEXT: Launch PLAN mode (launch.sh plan)"
fi
